...
@section('content')
    <div class="container">
        ...
        <!-- Page Heading -->
        <h1 class="text-left">New festival</h1>
        <hr>

        {{-- Page content--}}
        <div class="container">
            <div class="row">
                <form method="POST" action="{{ route('festival.sent') }}" enctype="multipart/form-data">
                    {{ csrf_field() }}

                    <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                        <input id="name" type="text" class="form-control" name="name" value="{{ old('name') }}" placeholder="Festival name" autofocus>
                        @if ($errors->has('name'))
                    ...
                    <div class="form-group{{ $errors->has('interval') ? ' has-error' : '' }}">
                        <input id="interval" type="text" class="form-control" value="{{ old('interval') }}" name="interval" placeholder="Interval" required>
                    ...
                    <div class="form-group{{ $errors->has('order') ? ' has-error' : '' }}">
                        <input id="order" type="number" class="form-control" value="{{ old('order') }}" name="order" placeholder="Order" required>
                        @if ($errors->has('order'))
                    ...
                    <button class="btn btn-lg btn-outline-danger" type="submit">Create festival</button>
                </form>
            </div>
        </div>
